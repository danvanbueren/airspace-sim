...
describe('actionPanelTemplates', () => {
    it('exposes the default operator template with both default panels', () => {
        const template = getActionPanelTemplate(ACTION_PANEL_TEMPLATE_IDS.DEFAULT_OPERATOR)
    ...
    it('clones template state without sharing references', () => {
        const clonedState = cloneActionPanelTemplateState(ACTION_PANEL_TEMPLATE_IDS.DEFAULT_OPERATOR)

        assert.notEqual(clonedState, getActionPanelTemplate(ACTION_PANEL_TEMPLATE_IDS.DEFAULT_OPERATOR).state)
        clonedState.panels[0].title = 'Changed title'
        assert.notEqual(
            clonedState.panels[0].title,
            getActionPanelTemplate(ACTION_PANEL_TEMPLATE_IDS.DEFAULT_OPERATOR).state.panels[0].title,
        )
    })
})
